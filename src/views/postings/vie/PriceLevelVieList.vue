<template>
  <div :class="prefixCls">
    <a-page-header v-if="isShow" title="闪断贴标准" :ghost="false" :class="`${prefixCls}__header`">
      <template #extra>
            ...
            <a-list-item-meta>
              ...
              <template #title>
                <p :class="`${prefixCls}__title`">
                  ...
                  <Tag v-if="isShow" class="mb-2" style="float: right">
                    {{ item.sort_no }}
                  </Tag>
                  <a-button
                    v-if="!isShow"
                    ...
                    type="primary"
                    size="small"
                    @click="addBrokerVie(item)"
                    :disabled="vielist.map((it) => it.price_level_id).indexOf(item.ID) != -1"
                  >
                    添加
                  </a-button>
                </p>
  ...
  export default defineComponent({
    ...
    setup({ vielist }, { emit }) {
      ...
      async function getData() {
        ...
        try {
          const dd = await priceLevelVieList({});
      ...
      function addBrokerVie(it: any) {
        emit('addBrokerVie', toRaw(it));
      }
